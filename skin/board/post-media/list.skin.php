...
$thumb_w = G5_IS_MOBILE ? 120 : 220;
$thumb_h = G5_IS_MOBILE ? 80 : 147;
$excerpt_len = G5_IS_MOBILE ? 60 : 90;

add_stylesheet('<link rel="stylesheet" href="'.$board_skin_url.'/style.css">', 0);
?>

<div class="board-wrap board-wrap--post-media" id="bo_list" style="width:<?php echo $width; ?>">
    ...
    <div class="board-list board-list--media">
        <?php if (count($list) == 0) { ?>
        ...
        ?>
            <li class="board-list__item <?php if ($list[$i]['is_notice']) echo 'board-list__item--notice bo_notice'; ?>">
                ...
                <?php } ?>
                <a href="<?php echo $list[$i]['href'] ?>" class="board-list__item-link board-list__item-link--media">
                    <span class="board-list__thumb-wrap board-list__thumb-wrap--media">
                        <?php echo $thumb_html; ?>
                    </span>
                    <span class="board-list__body board-list__body--media">
                        <span class="board-list__badges">
                            <?php if ($list[$i]['is_notice']) { ?><span class="notice_icon board-badge board-badge--notice">공지</span><?php } ?>
                            <?php if ($is_category && $list[$i]['ca_name']) { ?>
                            <span class="bo_cate_link board-list__cate board-list__cate--badge"><?php echo $list[$i]['ca_name'] ?></span>
                            <?php } ?>
                            <?php if ($list[$i]['icon_new']) echo '<span class="new_icon board-badge board-badge--new">N</span>'; ?>
                        </span>
                        <h2 class="board-title board-title--media">
                        ...
                        <span class="board-list__stats">
                            ...
                            <span class="cnt_cmt board-list__cmt"><i class="fa fa-commenting-o" aria-hidden="true"></i> <?php echo $list[$i]['wr_comment']; ?></span>
                            <?php } ?>

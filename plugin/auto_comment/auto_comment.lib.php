...
define('AUTO_COMMENT_VERSION', '1.1.10');
...
/**
 * onoff-g5-base 내장: plugin/auto_comment 폴더만 있으면 DB·스키마 자동 준비 (별도 install.php 불필요)
 *
 * @return bool 설치·마이그레이션 성공 여부
 */
function auto_comment_bootstrap()
{
    static $done = false;

    if ($done) {
        return function_exists('auto_comment_is_installed') && auto_comment_is_installed();
    }

    $done = true;

    if (!defined('G5_PLUGIN_PATH') || !defined('G5_TABLE_PREFIX')) {
        return false;
    }
    ...
    try {
        if (!auto_comment_is_installed()) {
            ...
            $schema = auto_comment_get_setting('schema_version', '');
            if ($schema === '' || version_compare((string) $schema, AUTO_COMMENT_VERSION, '<')) {
                auto_comment_update();
            }
        }
    } catch (Throwable $e) {
        if (function_exists('auto_comment_log') && auto_comment_table_exists(auto_comment_table('log'))) {
            auto_comment_log('bootstrap_failed', $e->getMessage(), 0);
        }

        return false;
    }

    return auto_comment_is_installed();
}
...
function auto_comment_seed_board_configs($replace)
{
    ...
    while ($board = sql_fetch_array($result)) {
        ...
        auto_comment_ensure_board_config($bo_table, $replace);
    }
}

function auto_comment_ensure_board_config($bo_table, $replace = false)
{
    global $g5;

    if (!auto_comment_is_installed() || empty($g5['board_table'])) {
        return false;
    }
    ...
    $board_row = sql_fetch(" select bo_table from {$g5['board_table']} where bo_table = '".auto_comment_escape($bo_table)."' limit 1 ", false);
...
function auto_comment_export_config()
{
    ...
    while ($row = sql_fetch_array($result)) {
        $interval_parts = auto_comment_board_interval_parts_from_minutes(isset($row['acb_interval_minutes']) ? (int) $row['acb_interval_minutes'] : 0);
        $data['boards'][] = array(
            ...
            'interval_minute_part' => $interval_parts['minutes']
        );
    }
...
function auto_comment_import_config($data)
{
    ...
    if (isset($data['boards']) && is_array($data['boards'])) {
        ...
        foreach ($data['boards'] as $board) {
            ...
            $import_row = array(
                'interval_schedule' => array_key_exists('interval_schedule', $board) ? $board['interval_schedule'] : (array_key_exists('midnight_schedule', $board) ? $board['midnight_schedule'] : 0)
            );
            if (isset($board['interval_hours'])) {
                $import_row['interval_hours'] = (int) $board['interval_hours'];
                $import_row['interval_minutes'] = isset($board['interval_minute_part']) ? (int) $board['interval_minute_part'] : 0;
            } else {
                $total_minutes = isset($board['interval_minutes']) ? (int) $board['interval_minutes'] : 0;
                $parts = auto_comment_board_interval_parts_from_minutes($total_minutes);
                $import_row['interval_hours'] = $parts['hours'];
                $import_row['interval_minutes'] = $parts['minutes'];
            }
            $parsed = auto_comment_parse_board_interval_from_row($import_row, $enabled);
            $interval_schedule = !empty($parsed['interval_schedule']) ? 1 : 0;
            $interval_minutes = isset($parsed['interval_minutes']) ? (int) $parsed['interval_minutes'] : 0;
...
function auto_comment_get_board_config($bo_table)
{
    ...
    $row = sql_fetch(" select * from ".auto_comment_table('board')." where bo_table = '".auto_comment_escape($bo_table)."' ");
    if (!$row) {
...
function auto_comment_board_interval_parts_from_minutes($minutes)
{
...
function auto_comment_parse_board_interval_from_row($row, $enabled)
{
    $enabled = (int) $enabled === 1;
    $interval_schedule = !empty($row['interval_schedule']) ? 1 : 0;
    $hours = isset($row['interval_hours']) ? max(0, min(168, (int) $row['interval_hours'])) : 0;
    $minutes_part = isset($row['interval_minutes']) ? max(0, min(59, (int) $row['interval_minutes'])) : 0;

    if ($enabled && ($hours > 0 || $minutes_part > 0)) {
        $interval_schedule = 1;
    }

    if (!$enabled || !$interval_schedule) {
        return array(
            'interval_schedule' => 0,
            'interval_minutes' => 0
        );
    ...
    if ($total < 1) {
        return array('error' => 'interval_empty');
    }

    return array(
        'interval_schedule' => 1,
        'interval_minutes' => auto_comment_normalize_interval_minutes($total)
    );
}
...
function auto_comment_get_icrm_api_base_url()
{
    if (function_exists('onoff_builder_config_api_base_url')) {
        $url = onoff_builder_config_api_base_url('auto_comment_api_base_url', '');
        if ($url !== '') {
            return $url;
        }
    }
...
function auto_comment_generate_gemini_content($board, $write, $tone = 'random')
{
    ...
    if (function_exists('onoff_builder_config_get')) {
        $model = onoff_builder_config_get('gemini_model', $model);
    }
    if ($model === '') {
        $model = 'gemini-2.0-flash-lite';
    }
    $api_key = trim(auto_comment_get_setting('ai_api_key', ''));
    if ($api_key === '' && function_exists('onoff_builder_config_get')) {
        $api_key = onoff_builder_config_get('gemini_api_key', '');
    }
    if ($api_key === '') {

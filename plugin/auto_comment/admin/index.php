...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($act === 'save_boards') {
        ...
        foreach ($boards as $bo_table => $row) {
            ...
            $parsed = auto_comment_parse_board_interval_from_row($row, $enabled);
            if (isset($parsed['error'])) {
                $interval_errors[] = $bo_table;
                continue;
            }
            $interval_schedule = (int) $parsed['interval_schedule'];
        ...
        if ($interval_errors) {
            ac_admin_redirect('boards', '간격 예약 ON인데 시간이 비어 있는 게시판이 있습니다: '.implode(', ', $interval_errors).'. 시간(시)·분을 입력하세요.');
        }
...
<div class="ac-container">
    ...
    <?php if (!$installed) { ?>
        ...
        <div class="ac-alert">
            ...
            간격 예약(시간·분 직접 입력), 조회수 트리거, 신규글 자동댓글 등은 <strong>이 화면의 「게시판설정」 탭</strong>에서 관리합니다.<br>
            그누보드 관리자의 <strong>게시판관리 &gt; 게시판 수정</strong> 화면에는 표시되지 않습니다.
        </div>
        <?php } ?>
        <p class="ac-help">자동댓글 DB가 아직 준비되지 않았습니다. 이 페이지를 새로고침하거나 아래 <strong>설치 시작</strong>을 누르세요. (onoff-g5-base는 보통 자동 설치됩니다.)</p>
        <p class="ac-help">실패 시 MySQL 권한·<code>data/cache</code> 쓰기 권한·<code>extend/auto_comment.extend.php</code> 존재 여부를 확인하세요.</p>
            ...
            <table class="ac-form-table">
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <span class="ac-help">AI 선택 시 iCRM 중앙 AI API에서 댓글을 생성합니다. 실제 API 사용료 × <?php echo function_exists('icrm_point_get_multiplier') ? (int) icrm_point_get_multiplier() : 5; ?>배가 포인트로 차감됩니다. 실패하면 템플릿으로 자동 대체됩니다.</span>
                    </td>
                </tr>
                <tr>
                    <th>온오프빌더 라이선스</th>
                    <td>
                        ...
                        <p class="ac-help" style="margin:0">공통 설정 파일(data/onoff-builder.config.php) 또는 온오프빌더 연동에 저장된 라이선스를 공유합니다.</p>
                        <?php } else { ?>
                        ...
                        <span class="ac-help">권장: data/onoff-builder.config.php 한 파일에 ONOFF_BUILDER_LICENSE_KEY를 입력하세요. 비워두면 기존 온오프빌더 연동 키를 사용합니다.</span>
                        <?php } ?>
                    </td>
                </tr>
    ...
    <?php } else if ($tab === 'boards') { ?>
        ...
        <p class="ac-help">게시판마다 <strong>댓글 간격을 직접</strong> 정합니다. <strong>관리대상 ON</strong> → <strong>간격 예약 ON</strong> → <strong>시간(시)·분</strong> 입력 후 저장. 시간만 넣어도 간격 예약이 자동으로 켜집니다. 예) 1시간마다: 1 / 0 · 6시간 30분마다: 6 / 30 · 15시간 20분마다: 15 / 20</p>
        <form method="post" action="<?php echo $action_url; ?>" id="ac-boards-form">
            <input type="hidden" name="ac_token" value="<?php echo $ac_token; ?>">
            <input type="hidden" name="act" value="save_boards">
            <table class="ac-form-table ac-board-table">
                <tr><th>게시판</th><th>성향</th><th>ON</th><th>간격예약</th><th>시간(시)</th><th>분</th><th>적용 간격</th><th>신규글</th><th>전략스캔</th><th>직접댓글</th><th>검수</th></tr>
                <?php
                auto_comment_ensure_board_columns();
                ...
                while ($bo = sql_fetch_array($result)) {
                    ...
                    $interval_schedule = $enabled && $cfg && isset($cfg['acb_midnight_schedule']) && (int) $cfg['acb_midnight_schedule'] === 1;
                    $stored_interval_minutes = $cfg && isset($cfg['acb_interval_minutes']) ? (int) $cfg['acb_interval_minutes'] : 0;
                    $interval_parts = auto_comment_board_interval_parts_from_minutes($stored_interval_minutes > 0 ? $stored_interval_minutes : 60);
                    $interval_hours = $interval_parts['hours'];
                    $interval_minutes_part = $interval_parts['minutes'];
                    $interval_preview = $interval_schedule && $stored_interval_minutes > 0
                        ? auto_comment_format_interval_minutes($stored_interval_minutes).'마다'
                        : ($interval_schedule ? '시간·분 입력 필요' : '-');
                    $auto_new_post = $enabled && !$interval_schedule && (!$cfg || !isset($cfg['acb_auto_new_post']) || (int) $cfg['acb_auto_new_post'] === 1);
                    $strategy_scan = $enabled && !$interval_schedule && (!$cfg || !isset($cfg['acb_strategy_scan']) || (int) $cfg['acb_strategy_scan'] === 1);
                ...
                <tr class="ac-board-row">
                    ...
                    <td><label><input type="checkbox" class="ac-board-child ac-board-interval" name="boards[<?php echo $bo['bo_table']; ?>][interval_schedule]" value="1" <?php echo $interval_schedule ? 'checked' : ''; ?><?php echo $child_disabled; ?>> ON</label></td>
                    <td><input type="number" class="ac-input ac-board-interval-field" name="boards[<?php echo $bo['bo_table']; ?>][interval_hours]" value="<?php echo (int) $interval_hours; ?>" min="0" max="168" step="1" placeholder="시" style="width:58px" title="시간(시)"<?php echo $child_disabled; ?>></td>
                    <td><input type="number" class="ac-input ac-board-interval-field" name="boards[<?php echo $bo['bo_table']; ?>][interval_minutes]" value="<?php echo (int) $interval_minutes_part; ?>" min="0" max="59" step="1" placeholder="분" style="width:58px" title="분"<?php echo $child_disabled; ?>></td>
                    <td class="ac-board-interval-preview ac-muted"><?php echo get_text($interval_preview); ?></td>
                    <td><label><input type="checkbox" class="ac-board-child ac-board-views-trigger" name="boards[<?php echo $bo['bo_table']; ?>][auto_new_post]" value="1" <?php echo $auto_new_post ? 'checked' : ''; ?><?php echo $child_disabled; ?><?php echo $interval_schedule ? ' disabled' : ''; ?>> ON</label></td>
                    <td><label><input type="checkbox" class="ac-board-child ac-board-views-trigger" name="boards[<?php echo $bo['bo_table']; ?>][strategy_scan]" value="1" <?php echo $strategy_scan ? 'checked' : ''; ?><?php echo $child_disabled; ?><?php echo $interval_schedule ? ' disabled' : ''; ?>> ON</label></td>
...
(function () {
    ...
    function formatIntervalPreview(hours, minutes) {
    ...
    function updateIntervalPreview(row) {
        if (!row) return;
        var preview = row.querySelector('.ac-board-interval-preview');
        if (!preview) return;
        var hoursInput = row.querySelector('input[name*="[interval_hours]"]');
        var minutesInput = row.querySelector('input[name*="[interval_minutes]"]');
        preview.textContent = formatIntervalPreview(
            hoursInput ? hoursInput.value : 0,
            minutesInput ? minutesInput.value : 0
        );
    }

    function maybeEnableIntervalFromFields(row) {
        if (!row) return;
        var master = row.querySelector('.ac-board-master');
        var interval = row.querySelector('.ac-board-interval');
        var hoursInput = row.querySelector('input[name*="[interval_hours]"]');
        var minutesInput = row.querySelector('input[name*="[interval_minutes]"]');
        if (!master || !master.checked || !interval || !hoursInput || !minutesInput) return;
    ...
    function syncBoardRow(master) {
        ...
        if (!row) return;
        var interval = row.querySelector('.ac-board-interval');
        var intervalFields = row.querySelectorAll('.ac-board-interval-field');
        ...
        for (var f = 0; f < intervalFields.length; f++) {
            intervalFields[f].disabled = !master.checked;
        }
        if (master.checked && interval && interval.checked) {
    ...
    var intervalBoxes = document.querySelectorAll('.ac-board-interval');
    for (var m = 0; m < intervalBoxes.length; m++) {
        intervalBoxes[m].addEventListener('change', function () {
    ...
    var intervalFieldsAll = document.querySelectorAll('.ac-board-interval-field');
    for (var n = 0; n < intervalFieldsAll.length; n++) {
        intervalFieldsAll[n].addEventListener('input', function () {
            var row = this.closest ? this.closest('.ac-board-row') : null;
            if (!row) return;
            maybeEnableIntervalFromFields(row);
            var master = row.querySelector('.ac-board-master');
            if (master) {
                syncBoardRow(master);
            } else {
    ...
    var boardsForm = document.getElementById('ac-boards-form');
    if (boardsForm) {
        boardsForm.addEventListener('submit', function (e) {
            var rows = document.querySelectorAll('.ac-board-row');
            for (var r = 0; r < rows.length; r++) {
                ...
                var hoursInput = row.querySelector('input[name*="[interval_hours]"]');
                var minutesInput = row.querySelector('input[name*="[interval_minutes]"]');
                if (!master || !master.checked || !interval || !interval.checked) continue;
                var hours = parseInt(hoursInput ? hoursInput.value : '0', 10) || 0;
                var minutes = parseInt(minutesInput ? minutesInput.value : '0', 10) || 0;
                if (hours < 1 && minutes < 1) {
                    e.preventDefault();
                    alert('간격 예약을 켠 게시판은 시간(시) 또는 분을 1 이상 입력하세요.');

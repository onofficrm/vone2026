...
<div class="ac-container">
    ...
    <?php if (!$installed) { ?>
        ...
        <form method="post" action="<?php echo $action_url; ?>">
            ...
            <button type="submit" class="ac-btn">설치 시작</button>
        </form>
    ...
    <?php } else if ($tab === 'boards') { ?>
        <p class="ac-help">게시판별 자동댓글 기능과 댓글 성향을 세밀하게 설정할 수 있습니다. 성향 프로필이 랜덤이면 댓글 흐름에 따라 여러 톤이 섞이고, 특정 성향을 선택하면 해당 게시판은 그 톤을 우선 사용합니다.</p>
        <form method="post" action="<?php echo $action_url; ?>">
            <input type="hidden" name="ac_token" value="<?php echo $ac_token; ?>">
            <input type="hidden" name="act" value="save_boards">
            <table class="ac-form-table ac-board-table">
                <tr><th>게시판</th><th>성향 프로필</th><th>관리대상</th><th>신규글 자동댓글</th><th>전략스캔</th><th>직접댓글</th><th>검수모드</th></tr>
                <?php
                auto_comment_ensure_board_columns();
                ...
                while ($bo = sql_fetch_array($result)) {
                    ...
                    $auto_new_post = $enabled && (!$cfg || !isset($cfg['acb_auto_new_post']) || (int) $cfg['acb_auto_new_post'] === 1);
                    $strategy_scan = $enabled && (!$cfg || !isset($cfg['acb_strategy_scan']) || (int) $cfg['acb_strategy_scan'] === 1);
                ...
                <tr class="ac-board-row">
                    ...
                    <td><label><input type="checkbox" class="ac-board-child" name="boards[<?php echo $bo['bo_table']; ?>][auto_new_post]" value="1" <?php echo $auto_new_post ? 'checked' : ''; ?><?php echo $child_disabled; ?>> ON</label></td>
                    <td><label><input type="checkbox" class="ac-board-child" name="boards[<?php echo $bo['bo_table']; ?>][strategy_scan]" value="1" <?php echo $strategy_scan ? 'checked' : ''; ?><?php echo $child_disabled; ?>> ON</label></td>

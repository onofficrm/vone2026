...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($act === 'save_boards') {
        ...
        foreach ($boards as $bo_table => $row) {
            ...
            $interval_schedule = isset($row['interval_schedule']) ? 1 : 0;
            $interval_hours = isset($row['interval_hours']) ? max(0, min(168, (int) $row['interval_hours'])) : 0;
            $interval_minutes_part = isset($row['interval_minutes']) ? max(0, min(59, (int) $row['interval_minutes'])) : 0;
            $interval_minutes = $interval_schedule ? auto_comment_normalize_interval_minutes(($interval_hours * 60) + $interval_minutes_part) : 0;
...
<div class="ac-container">
    ...
    <?php } else if ($tab === 'boards') { ?>
        ...
        <p class="ac-help">게시판별 자동댓글 기능과 댓글 성향을 설정합니다. <strong>간격 예약</strong> ON 후 <strong>시간·분</strong>을 입력하세요. 예) 1시간마다 → 시간 1 / 분 0, 6시간 30분마다 → 6 / 30, 15시간 20분마다 → 15 / 20. 조회수 기준(신규글·전략스캔)과 함께 쓰지 않습니다.</p>
        <form method="post" action="<?php echo $action_url; ?>">
            <input type="hidden" name="ac_token" value="<?php echo $ac_token; ?>">
            <input type="hidden" name="act" value="save_boards">
            <table class="ac-form-table ac-board-table">
                <tr><th>게시판</th><th>성향</th><th>ON</th><th>간격예약</th><th>시간</th><th>분</th><th>신규글</th><th>전략스캔</th><th>직접댓글</th><th>검수</th></tr>
                <?php
                auto_comment_ensure_board_columns();
                ...
                while ($bo = sql_fetch_array($result)) {
                    ...
                    $interval_schedule = $enabled && $cfg && auto_comment_board_uses_interval_schedule($cfg);
                    $stored_interval_minutes = $cfg && isset($cfg['acb_interval_minutes']) ? (int) $cfg['acb_interval_minutes'] : 60;
                    $interval_hours = (int) floor($stored_interval_minutes / 60);
                    $interval_minutes_part = $stored_interval_minutes % 60;
                ...
                <tr class="ac-board-row">
                    ...
                    <td><input type="number" class="ac-input ac-board-interval-field" name="boards[<?php echo $bo['bo_table']; ?>][interval_hours]" value="<?php echo (int) $interval_hours; ?>" min="0" max="168" style="width:52px"<?php echo $child_disabled; ?>></td>
                    <td><input type="number" class="ac-input ac-board-interval-field" name="boards[<?php echo $bo['bo_table']; ?>][interval_minutes]" value="<?php echo (int) $interval_minutes_part; ?>" min="0" max="59" style="width:52px"<?php echo $child_disabled; ?>></td>

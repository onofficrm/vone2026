...
define('AUTO_COMMENT_VERSION', '1.1.1');
...
function auto_comment_export_config()
{
    ...
    while ($row = sql_fetch_array($result)) {
        $data['boards'][] = array(
            ...
            'midnight_schedule' => isset($row['acb_midnight_schedule']) ? (int) $row['acb_midnight_schedule'] : 0
        );
    }
...
function auto_comment_import_config($data)
{
    ...
    if (isset($data['boards']) && is_array($data['boards'])) {
        ...
        foreach ($data['boards'] as $board) {
            ...
            $midnight_schedule = array_key_exists('midnight_schedule', $board) ? (!empty($board['midnight_schedule']) ? 1 : 0) : 0;
...
function auto_comment_board_uses_midnight_schedule($cfg)
{
    return is_array($cfg) && isset($cfg['acb_midnight_schedule']) && (int) $cfg['acb_midnight_schedule'] === 1;
}
...
function auto_comment_board_uses_views_trigger($cfg)
{
    ...
    return !auto_comment_board_uses_midnight_schedule($cfg);
}

function auto_comment_midnight_target_comments_for_board($cfg)
{
...
function auto_comment_schedule_for_post($board, $wr_id, $w)
{
    ...
    if (auto_comment_board_uses_midnight_schedule($cfg)) {
        return;
    }
...
function auto_comment_midnight_schedule_for_write($board, $write, $cfg, $midnight_base, $slot_index)
{
    $bo_table = $board['bo_table'];
    $wr_id = (int) $write['wr_id'];
    $group = !empty($cfg['acb_template_group']) ? $cfg['acb_template_group'] : 'default';
    $min_delay = max(0, (int) $cfg['acb_min_delay']);
    $max_delay = max($min_delay, (int) $cfg['acb_max_delay']);
    $delay_minutes = $slot_index === 0 ? 0 : mt_rand($min_delay, $max_delay) + (($slot_index - 1) * mt_rand(1, 5));
    $scheduled_at = date('Y-m-d H:i:s', strtotime($midnight_base) + ($delay_minutes * 60));
    ...
    try {
        ...
        auto_comment_log('midnight_skip', $bo_table.' #'.$wr_id.' '.$e->getMessage(), 0);
        return 0;
    }
    ...
    $review_mode = isset($cfg['acb_review_mode']) && (int) $cfg['acb_review_mode'] === 1;
    $status = ($review_mode || $quality['score'] < 65) ? 'review' : 'pending';
    $target = auto_comment_midnight_target_comments_for_board($cfg);
    $existing_count = auto_comment_existing_count_for_post($bo_table, $wr_id);
    $meta = '자정 예약: 매일 00:00 기준 글당 '.$target.'개 중 '.($existing_count + 1).'번째 / '.auto_comment_quality_meta($content);
    if ($review_mode) {
        $meta .= ' / 게시판 검수모드';
    }
    ...
    auto_comment_log('midnight_queue', $bo_table.' #'.$wr_id.' 자정 예약 '.($existing_count + 1).'/'.$target, $queue_id);

    return 1;
}

function auto_comment_midnight_schedule_scan()
{
    global $g5;
    ...
    if (auto_comment_get_setting('midnight_last_run_date', '') === $today) {
        return 0;
    }
    ...
    while ($cfg = sql_fetch_array($result)) {
        ...
        while ($write = sql_fetch_array($posts)) {
            ...
            for ($i = 0; $i < $need; $i++) {
                $created += auto_comment_midnight_schedule_for_write($board, $write, $cfg, $midnight_base, $existing_count + $i);
            }
        }
    }

    auto_comment_set_setting('midnight_last_run_date', $today);
    auto_comment_log('midnight_scan', '자정 예약 스캔 완료: 확인 '.$checked.'개, 예약 '.$created.'개', 0);
...
function auto_comment_maybe_run_worker()
{
    ...
    $max_seconds = max(1, min(5, (int) auto_comment_get_setting('max_run_seconds', '2')));
    try {
        auto_comment_midnight_schedule_scan();
